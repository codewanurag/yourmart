...
import { User, Mail, Phone, MapPin, Edit2, Store, Check, Loader2, Camera, LogOut } from 'lucide-react'
...
export default function ProfilePage() {
  ...
  const handleBecomeSeller = async (e) => {
    e.preventDefault()
    if (!sellerForm.storeName) { toast.error('Store name required'); return }
    setSaving(true)
    try {
  ...
  const initials = user?.name?.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() || 'YM'

  const inputClass = `w-full px-4 py-3 rounded-xl text-sm transition-all outline-none ${
    isDark ? 'bg-ink3 border border-white/[0.07] text-cream placeholder:text-smoke focus:border-gold/40'
           : 'bg-white border border-lt_border text-lt_text placeholder:text-lt_muted focus:border-gold/40'
  }`

  return (
    <div className="animate-slide-up max-w-2xl mx-auto">
      <h1 className={`font-serif text-2xl font-bold mb-6 ${isDark ? 'text-cream' : 'text-lt_text'}`}>My Profile</h1>
      ...
      <div className={`rounded-3xl border p-6 mb-5 ${isDark ? 'bg-ink2 border-white/[0.07]' : 'bg-white border-lt_border'}`}>
        <div className="flex flex-col sm:flex-row gap-5 items-start sm:items-center mb-6">
          ...
          <div className="flex-1">
            <h2 className={`font-serif text-xl font-bold ${isDark ? 'text-cream' : 'text-lt_text'}`}>{user?.name}</h2>
            <p className={`text-sm ${isDark ? 'text-dim' : 'text-lt_muted'}`}>{user?.email}</p>
            {user?.isSeller && (
              <span className="inline-flex items-center gap-1.5 mt-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-gold/10 text-gold border border-gold/20">
                <Store size={11} /> Verified Seller — {user.sellerInfo?.storeName}
              </span>
            )}
          </div>
        ...
        {editing ? (
          <div className="flex flex-col gap-4">
            {[
              ...
              { key: 'location', label: 'Location',     type: 'text',  ph: 'City, State' },
              ...
              <div key={key}>
                ...
                {textarea
                  ? <textarea value={form[key]} placeholder={ph} rows={2}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              ...
              <div key={label} className={`flex items-start gap-3 p-4 rounded-xl border ${isDark ? 'bg-ink3 border-white/[0.07]' : 'bg-gray-50 border-lt_border'}`}>
                <Icon size={16} className="text-gold flex-shrink-0 mt-0.5" />
                <div>
                  <p className={`text-xs font-medium ${isDark ? 'text-muted' : 'text-lt_muted'}`}>{label}</p>
                  <p className={`text-sm mt-0.5 ${isDark ? 'text-cream' : 'text-lt_text'}`}>{value}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        {[
          ...
          ...(user?.isSeller ? [{ label: 'Dashboard', icon: '📊', path: '/seller/dashboard' }] : [{ label: 'Become Seller', icon: '🏪', action: () => setShowSeller(true) }]),
        ].map(item => (
          <button key={item.label}
            ...
            <span className={`text-xs font-medium ${isDark ? 'text-dim' : 'text-lt_muted'}`}>{item.label}</span>
          </button>
        ))}
      </div>
      ...
      {showSeller && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm"
          ...
          <div className={`w-full max-w-md rounded-3xl border p-6 shadow-2xl ${isDark ? 'bg-ink2 border-white/[0.07]' : 'bg-white border-lt_border'}`}
            ...
            <form onSubmit={handleBecomeSeller} className="flex flex-col gap-4">
              {[
                { key: 'storeName', label: 'Store Name',        ph: 'e.g. Rajwadi Fabrics' },
                { key: 'storeTag',  label: 'Store Tag Line',    ph: 'e.g. Premium Handlooms' },
                { key: 'storeDesc', label: 'Store Description', ph: 'Tell buyers about your store', textarea: true },
              ].map(({ key, label, ph, textarea }) => (
                <div key={key}>
                  <label className={`block text-xs font-medium mb-1.5 ${isDark ? 'text-dim' : 'text-lt_muted'}`}>{label}</label>
                  {textarea
                    ? <textarea value={sellerForm[key]} placeholder={ph} rows={2}
                        onChange={e => setSellerForm(f => ({ ...f, [key]: e.target.value }))}
                        className={`${inputClass} resize-none`} />
                    : <input value={sellerForm[key]} placeholder={ph}
                        onChange={e => setSellerForm(f => ({ ...f, [key]: e.target.value }))}
                        className={inputClass} />
                  }
                </div>
              ...
              <div className="flex gap-3 mt-2">
                <button type="button" onClick={() => setShowSeller(false)}
