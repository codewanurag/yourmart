...
import { Play, TrendingUp, Zap, Star, ArrowRight, ChevronRight } from 'lucide-react'
    ...
    <div className="animate-slide-up max-w-3xl mx-auto xl:max-w-none">
        ...
        {['forYou', 'trending', 'newArrivals', 'following'].map(tab => (
          <button key={tab}
            ...
            className={`px-5 py-2.5 text-[0.83rem] font-medium tracking-wide relative transition-colors whitespace-nowrap
              ${activeTab === tab
                ? isDark ? 'text-cream after:absolute after:bottom-[-1px] after:left-0 after:right-0 after:h-px after:bg-gradient-to-r after:from-gold after:to-gold2'
            ...
            {{ forYou: 'For You', trending: 'Trending', newArrivals: 'New Arrivals', following: 'Following' }[tab]}
          </button>
        ))}
      </div>
      ...
      <div className="flex gap-3 mb-6 overflow-x-auto pb-2 scrollbar-none">
        {STORIES.map((story, i) => (
          <div key={i} className="flex flex-col items-center gap-1.5 cursor-pointer flex-shrink-0"
            ...
            <div className={`w-14 h-14 rounded-full p-[2px] transition-transform hover:scale-105
              ${story.isAdd
                ? isDark ? 'border-2 border-dashed border-smoke' : 'border-2 border-dashed border-lt_border'
                : story.isLive
                  ? 'ring-spin'
                  : ''
              }`}
              style={!story.isAdd && !story.isLive
                ? { background: `conic-gradient(#C9A84C 0%, #E8C96A 40%, #E05C2A 70%, #C9A84C 100%)`, padding: '2px' }
                : story.isLive
                  ? { background: 'conic-gradient(#E05050 0%, #FF8080 50%, #E05050 100%)', padding: '2px', borderRadius: '50%' }
                  : {}
              }>
              <div className={`w-full h-full rounded-full flex items-center justify-center text-base font-bold border-2
                ${isDark ? 'bg-ink3 border-ink' : 'bg-lt_bg border-lt_bg'}`}
                style={story.color ? { background: story.color + '22', color: story.color } : {}}>
                {story.isAdd ? '+' : story.seller
                  ? (story.seller.avatar
                    ? <img src={story.seller.avatar} className="w-full h-full rounded-full object-cover" alt={story.name} />
                    : story.name[0])
                  : story.name[0]}
              </div>
            </div>
            <span className={`text-[0.65rem] w-14 text-center truncate ${isDark ? 'text-muted' : 'text-lt_muted'}`}>
      ...
      >
        <div className="flex items-stretch relative">
          ...
          {/* Info */}
          <div className="flex-1 p-5 flex flex-col justify-center" style={{ position: 'relative', zIndex: 1 }}>
            <div className="text-[0.65rem] font-bold text-gold tracking-[0.2em] uppercase mb-2">
              ✦ Featured Live Session
            </div>
            <h2 className={`font-serif text-base sm:text-lg font-bold leading-tight mb-3 ${isDark ? 'text-parchment' : 'text-lt_text'}`}>
              {heroSession.title}
            </h2>
            <div className="flex gap-4 flex-wrap mb-4">
              <span className={`text-xs flex items-center gap-1 ${isDark ? 'text-dim' : 'text-lt_muted'}`}>
                <span className="text-red-400">●</span> {heroSession.viewers.toLocaleString()} watching
              </span>
              <span className={`text-xs flex items-center gap-1 ${isDark ? 'text-dim' : 'text-lt_muted'}`}>
                🏪 {heroSession.seller.sellerInfo.storeName}
              </span>
              <span className={`text-xs flex items-center gap-1 ${isDark ? 'text-dim' : 'text-lt_muted'}`}>
                ★ {heroSession.seller.sellerInfo.rating}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-gold to-gold2 text-ink text-xs font-bold">
                <Play size={12} fill="currentColor" /> Join Live
              </div>
              <div className={`text-xs px-3 py-2 rounded-xl border ${isDark ? 'border-white/[0.1] text-dim' : 'border-lt_border text-lt_muted'}`}>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className={`font-serif text-lg font-bold ${isDark ? 'text-cream' : 'text-lt_text'}`}>Live Sessions</h2>
          </div>
          ...
          {MOCK_LIVE_SESSIONS.map((session, i) => (
            <motion.div
              ...
              className={`rounded-2xl overflow-hidden border cursor-pointer group transition-all hover:-translate-y-1 hover:shadow-xl
                ${isDark ? 'border-gold/15 hover:border-gold/30' : 'border-lt_border hover:border-gold/30'}`}
            ...
            >
              <div className="relative h-36 overflow-hidden">
                ...
                <div className="absolute top-2.5 left-2.5 flex items-center gap-1.5 bg-black/70 border border-red-500/40 rounded px-2 py-1 text-[0.6rem] font-bold text-red-400 tracking-widest">
                  <span className="w-1.5 h-1.5 rounded-full bg-red-500 live-dot" />LIVE
                </div>
                <div className="absolute top-2.5 right-2.5 bg-black/60 rounded px-2 py-1 text-[0.68rem] font-mono text-dim">
                  👁 {session.viewers.toLocaleString()}
                </div>
                <div className="absolute bottom-2.5 left-2.5 right-2.5">
                  <p className="text-parchment text-xs font-semibold line-clamp-1 mb-0.5">{session.title}</p>
                  <p className="text-muted text-[0.65rem]">{session.seller.sellerInfo.storeName}</p>
                </div>
